...
export default function SalesMixPage() {
  const [data, setData] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    const d = await fetchKPI("sales_mix");
    setData(d);
    setLoading(false);
  }, []);
  ...
  return (
    <div style={pageStyle}>
      <PageHeader title="🟠 KPI 3: Sales Mix" subtitle="Channel-wise sales mix with MoM comparison (Revenue in Lakhs)" />

      <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "24px", maxWidth: "900px" }}>
        
        {/* PART A: MoM COMPARISON STACKED BARS */}
        <div style={cardStyle}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #E5E7EB", paddingBottom: "16px", marginBottom: "24px" }}>
            <h3 style={cardHeaderStyle}>Monthly Channel Revenue Mix (₹ Lakhs)</h3>
