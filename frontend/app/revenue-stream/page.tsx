...
export default function RevenueStreamPage() {
  const [data, setData] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    try {
      const res = await fetchKPI("revenue_stream");
      setData(res);
    } catch (e) {
      console.error(e);
    }
  ...
  if (!data || data.error) return <div style={pageStyle}><p>⚠️ Cannot load Revenue KPI data. Backend returned an error.</p></div>;

  return (
    <div style={pageStyle}>
      <PageHeader title="🟠 KPI 5: Revenue Stream" subtitle="Revenue streams (TA Fees / Management Fees / Profit Incentive) with MoM and YoY comparison (₹ in Lakhs)" />

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px", maxWidth: "1400px" }}>
        
        {/* PART A: MoM COMPARISON STACKED BARS (Left Chart) */}
        <div style={cardStyle}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #E5E7EB", paddingBottom: "16px", marginBottom: "24px" }}>
            <h3 style={cardHeaderStyle}>Monthly Revenue Stream Mix</h3>
            
            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end" }}>
                <span style={{ fontSize: "12px", color: "#6B6B6B", marginBottom: "4px" }}>MoM Growth</span>
                <span style={{ 
                    background: data.mom_pct >= 0 ? "#F0FDF4" : "#FEF2F2", 
                    color: data.mom_pct >= 0 ? "#16A34A" : "#DC2626", 
                    padding: "6px 16px", 
                    borderRadius: "20px", 
                    fontSize: "16px", 
                    fontWeight: 700,
                    ...
                    {data.mom_pct >= 0 ? "↗" : "↘"} {data.mom_pct > 0 ? "+" : ""}{data.mom_pct}%
                </span>
            </div>
          </div>
          ...
          <div style={{ height: "450px", width: "100%" }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data.mom_chart || []} margin={{ top: 20, right: 30, left: 0, bottom: 20 }} barSize={100}>
                ...
                {/* 3 stacked segments specifically ordered for KPI 5 */}
                <Bar dataKey="TA Fees" stackId="a" fill="#E4572E">
                    <LabelList dataKey="TA Fees" position="center" fill="#FFFFFF" fontSize={13} fontWeight={600} formatter={(v: any) => v && v > 0 ? `${v}L` : ''} />
                </Bar>
                <Bar dataKey="Management Fees" stackId="a" fill="#1A1A1A">
                    <LabelList dataKey="Management Fees" position="center" fill="#FFFFFF" fontSize={13} fontWeight={600} formatter={(v: any) => v && v > 0 ? `${v}L` : ''} />
                </Bar>
                <Bar dataKey="Profit Incentive" stackId="a" fill="#9CA3AF">
                    <LabelList dataKey="Profit Incentive" position="center" fill="#FFFFFF" fontSize={13} fontWeight={600} formatter={(v: any) => v && v > 0 ? `${v}L` : ''} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        {/* PART B: YoY COMPARISON STACKED BARS (Right Chart) */}
        <div style={cardStyle}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #E5E7EB", paddingBottom: "16px", marginBottom: "24px" }}>
            <h3 style={cardHeaderStyle}>Annual YoY Revenue Stream Mix</h3>
          ...
          <div style={{ height: "450px", width: "100%" }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data.yoy_chart || []} margin={{ top: 20, right: 30, left: 0, bottom: 20 }} barSize={100}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#E5E7EB" />
                <XAxis dataKey="month" tick={{ fontSize: 14, fill: "#1A1A1A", fontWeight: 600 }} axisLine={{ stroke: "#E5E7EB" }} tickLine={false} dy={10} />
                <YAxis tick={{ fontSize: 12, fill: "#6B6B6B" }} axisLine={{ stroke: "#E5E7EB" }} tickLine={false} dx={-10} tickFormatter={(val) => `${val}L`} />
                <RechartsTooltip contentStyle={tooltipStyle} formatter={(val: any) => [`₹${val}L`, "Revenue"]} />
...
function Loading() { return <div style={{ ...pageStyle, color: "#6B6B6B", display: "flex", justifyContent: "center", alignItems: "center", height: "50vh" }}><div style={{ fontSize: "16px", fontWeight: 500 }}>Loading Managerial Execution Charts...</div></div>; }
